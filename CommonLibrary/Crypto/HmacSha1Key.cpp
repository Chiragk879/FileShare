...
#define _HMAC_SHA1_KEY_SIZE 20

using namespace IMFS;

const size_t HmacSha1Key::HMAC_SHA1_KEY_SIZE = _HMAC_SHA1_KEY_SIZE;

struct hmac_sha1_key_blob_t
{
    BLOBHEADER hdr;
    DWORD dwKeySize;
    BYTE keyData[1];
...
HmacSha1Key::HmacSha1Key(HCRYPTPROV hProv, const std::vector<uint8_t>& key) :
m_hKey(0)
{
    hmac_sha1_key_blob_t* keyBlob = nullptr;

    DWORD keySize = static_cast<DWORD>(key.size());
    DWORD keyBlobSize = keySize + sizeof(hmac_sha1_key_blob_t) - 1;
    keyBlob = static_cast<hmac_sha1_key_blob_t*>(malloc(keyBlobSize));
    if (keyBlob == nullptr)
    {
...
bool HmacSha1Key::CalcHmac(const void* data, size_t size, std::vector<uint8_t>& result)
{
    ...
    // 4 - Configure HMAC to use SHA1 - set HmacInfo parameters to HMAC object by handle hHmacHash
    ...
    // 6 - Get length of result (should be 160/8=20 bytes)
    ...
    if (dwDataLen != HMAC_SHA1_KEY_SIZE)
    {
        IMLOG_ERROR(L"%s: HMAC size should be 20 bytes", __FUNCTIONW__);
        goto ErrorExit;
    }
#endif
